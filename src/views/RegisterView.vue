...
<template>
  <div class="min-h-screen bg-slate-50 flex items-center justify-center p-6 selection:bg-slate-200">
    ...
    <div :class="['w-full max-w-lg bg-white p-10 md:p-12 rounded-sm shadow-sm transition-all duration-1000 ease-out transform', isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
      
      <div class="text-center mb-10">
        <h1 class="text-3xl font-medium text-slate-900 mb-2">Create Account</h1>
        <p class="text-slate-500 font-light">Join us to explore premium essentials.</p>
      </div>
      ...
      <form @submit.prevent="handleRegister" class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row gap-6">
          ...
          <div class="space-y-2 text-left flex-1">
            <label for="surname" class="block text-sm font-medium text-slate-700">Last Name</label>
            <input 
              type="text" 
              id="surname" 
              v-model="surname" 
      ...
      <div class="mt-8 text-center">
        <p class="text-sm text-slate-500">
          ...
          <RouterLink :to="{ name: 'login' }" class="font-medium text-slate-900 border-b border-transparent hover:border-slate-900 transition-all duration-300 ease-in-out pb-0.5">
            Log in
          </RouterLink>
        </p>
      </div>
